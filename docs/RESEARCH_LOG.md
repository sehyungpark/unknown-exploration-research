...
## 2026-09-16 — Algorithm C Stage 6 Full Development A/C Regression

- **Verified shared-snapshot architecture:** A test-only complete-run harness evaluates frozen exhaustive Algorithm A and one fresh stateful Algorithm C instance on the identical belief object and robot coordinate at every planning snapshot. It verifies that neither planner changes the shared environment, compares all required outputs and invariants, and only then executes the one agreed path followed by one physical arrival scan. A first disagreement or exception stops before movement; no exhaustive fallback exists.
- **Fixed-fixture development gate:** All **7/7** repository fixtures reached the same explicit terminal snapshot within their unchanged fixture limits. **89 shared planning snapshots** were checked. Target mismatches: **0**; status/sequence/termination mismatches: **0**; selected gain mismatches: **0**; selected distance mismatches: **0**; selected score mismatches: **0**; path mismatches: **0**; maintained-bound violations: **0**.
- **Fixed-fixture traces:** Every row below had zero target, status/termination, selected gain/distance/score/path, and maintained-bound mismatches.
  - `open`: 15 snapshots; safe skip observed; `[(9,10), (8,10), (7,10), (7,7), (7,6), (12,6), (13,6), (13,12), (13,13), (7,14), (6,14), (3,7), (12,3), (16,12), STOP]`.
  - `single_room`: 13 snapshots; safe skip observed; `[(9,10), (8,10), (7,10), (7,7), (7,6), (12,6), (13,6), (13,12), (13,13), (7,14), (6,14), (5,6), STOP]`.
  - `corridor`: 5 snapshots; safe skip observed; `[(10,6), (10,11), (10,15), (10,18), STOP]`.
  - `dead_end`: 7 snapshots; safe skip observed; `[(10,6), (10,11), (9,12), (5,12), (10,16), (10,18), STOP]`.
  - `separated_rooms`: 12 snapshots; safe skip observed; `[(12,5), (14,11), (13,12), (12,13), (7,13), (5,9), (5,7), (5,6), (5,13), (14,14), (14,5), STOP]`.
  - `clutter`: 12 snapshots; safe skip observed; `[(6,5), (10,11), (11,12), (12,11), (13,10), (16,7), (7,15), (12,16), (6,16), (7,6), (8,6), STOP]`.
  - `maze_like`: 25 snapshots; safe skip observed; `[(8,3), (12,3), (16,3), (17,4), (17,5), (10,7), (6,7), (2,7), (2,9), (9,11), (13,11), (17,11), (17,13), (9,13), (8,13), (7,13), (2,16), (2,17), (4,17), (6,17), (8,17), (10,17), (12,17), (14,17), STOP]`.
- **Frozen-v2 development gate:** All **60/60** canonical `experiment-0-random-v2` maps were regenerated from their frozen records and reverified for seed, start, cycle limit, regenerated map hash, and canonical ground-truth hash before execution. Every run reached the shared terminal snapshot within its unchanged recorded limit. **1,096 shared planning snapshots** were checked, with all mismatch and maintained-bound violation counts equal to **0**.
- **Candidate and diagnostic validation:** On all **1,185** snapshots across **67** complete runs, C's candidate domain/order and every current distance matched A exactly. Every available C snapshot-entry bound dominated A's current exact gain. Every exact-evaluated C candidate matched A's gain and score and had a tight post-refresh cache; every bound-only candidate remained unrefreshed with no current exact value. Every selected C candidate was current-exact.
- **Failure classifications observed:** No supported-run `ChangeAwareGainBoundViolation` occurred. No unsupported-assumption or lifecycle failure occurred. No target, status, termination, selected-value, path, candidate-domain, distance, cache, or bound failure occurred.
- **Lazy mechanism exercised:** Both fixture and frozen-random workloads contained at least one selected snapshot where C performed fewer exact gain evaluations than A. This is only a structural correctness sanity check; it is not a wall-clock, speedup, memory, or efficiency result.
- **Interpretation:** Algorithm C has passed the full development A/C exact-equivalence regression under the frozen simulator assumptions. This empirical validation does not replace or globally prove the conditional admissibility theorem.
- **Full suite:** **165 tests passed** on 2026-09-16, including the unchanged full A/B regressions and two new Stage 6 complete-run regressions.
- **Scope:** The formal A/B/C Experiment 0 runner and immutable failure-artifact writer were not added. Experiment 0 was not executed. Algorithms A/B, Algorithm C semantics, simulator, fixtures, frozen dataset/config, cycle limits, and preregistration were not changed.
- **No research experiment executed:** Stage 6 is a development correctness regression, so no entry was added to `docs/EXPERIMENT_LOG.md`.

## 2026-09-16 — Stage 7 Formal Experiment 0 Infrastructure

- **Verified infrastructure:** `experiment-0-runner-v1` evaluates Algorithms A, B, and C in that order on one shared belief object and robot coordinate. It verifies that planning did not mutate the snapshot, performs every required comparison and independent invariant check, validates and persists the terminal-inclusive cycle record, and moves along the one agreed A/B/C path only after the complete comparison passes.
- **Verified canonical logging:** The serializer implements exactly the frozen per-cycle field names, converts coordinates to `[row, col]`, uses `SELECTED` / `EXPLORATION_COMPLETE`, rejects non-finite numbers and malformed state/count/status/flag combinations, and emits compact `sort_keys=True`, `separators=(",", ":")`, `allow_nan=False` JSON followed by one LF. Invocation directories are non-overwriting and contain `cycles.jsonl`, a manifest, and a summary derived from the actual records.
- **Verified direct validation:** B and C candidate domain/order and exact current distances are compared with A. Each available snapshot-entry bound and every post-plan maintained bound are compared directly with A's current exact gain. B and C tie certificates are reconstructed independently from the selected current-exact record, current maintained bounds, current distances, `optimistic_rank_state`, and `tie_aware_rank_certificate`; terminal certification requires no candidates or all current bounds zero. C additionally runs `cache.validate()` and cross-checks cached/inverse membership totals.
- **Verified decrement diagnostic:** `c_bound_decrement_count` is the sum of the sizes of the pre-C-plan inverse-incidence memberships for every synchronized newly-known cell. The inverse snapshot is captured before C performs revelation synchronization; the first planner call normally has an empty pre-plan inverse index and records zero.
- **Verified immutable failure evidence:** Every supported classification produces the frozen six-file directory with canonical ground truth and belief text/hash, deterministic candidate ordering, available A/B/C entry/post-bound/exact/rank diagnostics, complete B scalar cache and C cached-set/bound/inverse/reported/snapshot state, exception details, and a guarded single-map reproduction command. A deterministic failure ID is distinct from the invocation ID, an existing directory is refused, and the duplicate-write regression confirmed every existing byte remained unchanged.
- **Verified fault handling:** Forced B and C target mismatches, B and C underbounds, C inverse-index corruption, a supported `ChangeAwareGainBoundViolation`, safety-limit exhaustion, and a malformed cycle record all produced the intended stable classification and evidence in temporary output locations. Forced target mismatches and all pre-comparison failures produced zero movement. The supported C violation retained its detached diagnostic state and no oracle fallback was used.
- **Verified execution guard and scope:** The formal CLI requires `--execute-preregistered`, supports `--only-map`, verifies the seven-fixture order and all 60 random-v2 seed/start/limit/regenerated/canonical-hash inputs, and refuses an unguarded invocation before constructing output. Stage 7 tests used only the explicitly non-preregistered `stage7-smoke` map and workspace-local temporary directories.
- **Full suite:** **197 tests passed** on 2026-09-16, including 32 Stage 7 tests and all unchanged Algorithm A, A/B, A/C, Stage 5 hardening, dataset, simulator, and sensing regressions.
- **Research status unchanged:** This was infrastructure implementation and fault-injected validation, not the preregistered experiment. The real Experiment 0 was not executed, no real `results/experiment_0` research result was written, frozen Experiment 0 documents/config were not changed, Algorithm A/B/C and simulator semantics were not changed, and no entry was added to `docs/EXPERIMENT_LOG.md`. Stage 8 remains the first formal Experiment 0 execution.

## 2026-09-16 — First Formal Preregistered Experiment 0 Result

- **Verified empirical conclusion:** Experiment 0 exact-equivalence correctness gate **PASS**. At accepted revision `b8597a9453fe2a6e011098c8da2c9a0a96e2fb7a`, Algorithms B and C matched exhaustive Algorithm A on every one of the **1,185** preregistered planning snapshots and on all **67** complete target/STOP sequences from seven deterministic fixtures plus 60 frozen `experiment-0-random-v2` maps.
- **No observed counterexample:** Target, status/termination, selected gain/distance/score/path, candidate-domain/distance, B/C bound, B/C tie-certificate, C cache/index, supported `ChangeAwareGainBoundViolation`, unsupported lifecycle, safety-limit, malformed-record, and unexpected-failure counts were all **0**. Every map ended with one explicit shared `EXPLORATION_COMPLETE` terminal snapshot.
- **Diagnostic counts:** Total exact-gain evaluations were A **215,939**, B **40,764**, and C **20,578**. These counts are retained as correctness diagnostics only.
- **Hypothesis status:** The preregistered empirical exact-equivalence hypothesis is supported on the frozen correctness dataset. This does not convert the conditional theorem into a global proof and does not remove its static-world, monotone-belief, immutable-viewpoint, fixed-sensor, and frozen tie-rule assumptions.
- **Research boundary:** Experiment 0 is a correctness experiment. No wall-clock, memory, speedup, or efficiency conclusion is drawn from this result. The next research task is to design and preregister a separate efficiency evaluation before running it.
- **Preserved evidence:** `results/experiment_0/runs/experiment0-formal-20260916-001/` contains the unmodified formal `cycles.jsonl`, `manifest.json`, and `summary.json`; no failure artifact exists for the passing invocation.

## 2026-09-16 — Experiment 1 Efficiency Evaluation Preregistration

- **Preregistered question:** Determine whether Candidate 1's change-aware Algorithm C reduces canonical exact visibility work and total planning time relative to stale-scalar Algorithm B while retaining exact A/B/C decisions, and quantify its maintenance and memory cost.
- **Frozen dataset:** `experiment-1-efficiency-v1` uses master seed `20260916`, 20×20/30×30/40×40 maps, sparse/medium/dense Bernoulli occupancy, and three accepted maps per stratum. Structural generation accepted 27 candidates and rejected 0; this is a dataset observation, not an algorithm result.
- **Frozen evidence hierarchy:** deterministic exact-evaluation/ray/supercover/probe counters and structural storage are primary algorithmic scaling evidence; total planner wall time is primary machine-dependent evidence; internal decomposition and separate `tracemalloc` are secondary diagnostics.
- **Frozen comparison:** C/B is primary. Map is the statistical unit. Nine first-in-stratum maps receive one discarded warm-up and six balanced-order timed repetitions. Paired log-ratio bootstrap uses 10,000 map resamples and seed `20260916`.
- **Frozen decision rule:** GO requires intact correctness, strictly lower aggregate exact visibility work for C than B, and a C/B planning-time interval entirely below 1. MODIFY covers useful deterministic pruning with inconclusive/absorbed timing. DROP covers no deterministic visibility-work reduction, or consistently slower C with no structural scaling value. Correctness failure instead reopens correctness.
- **Hypothesis status unchanged:** Experiment 1 has **not** been executed. No efficiency, speed, memory, or scaling conclusion has changed, and the empirical-value gate remains **OPEN**.
- **Scope protection:** No Algorithm A/B/C, simulator, Experiment 0 artifact, or formal result was changed. Stage 9 performed only structural map generation and deterministic regeneration tests; no planner, sensing, timing, memory tracing, or work-counter workload ran on the new maps.
- **Verification:** All 9 focused dataset-regeneration tests passed, followed by the complete 206-test suite with exit code 0 on 2026-09-16. This verification does not constitute Experiment 1 execution.

## 2026-09-17 — Experiment 1 Pre-execution Amendment 1

- **Ambiguity detected before execution:** The original Stage 9 wording allowed a possible overlap between MODIFY and DROP when C reduced all three deterministic exact-visibility work totals but its C/B planning-time interval lay entirely above 1.0. No Experiment 1 timing, memory, work, slack, or planner-performance result had been observed when this ambiguity was corrected.
- **Deterministic superseding rule:** Primary `R=8` correctness failure yields `CORRECTNESS_REOPENED` and invalidates performance interpretation. Otherwise, failure of the all-three strict aggregate visibility-work condition yields DROP. If that condition passes, `upper < 1.0` yields GO, `lower <= 1.0 <= upper` yields MODIFY, and `lower > 1.0` yields DROP. Endpoint equality therefore belongs to MODIFY, and no subjective post-result override is permitted.
- **Interpretation boundary:** Structural cache/index size, separate `tracemalloc` results, candidate-density scaling, and sensor-range scaling remain required reported tradeoffs and secondary interpretation, but do not independently change GO/MODIFY/DROP.
- **Unchanged scope:** Dataset, maps, timing and sensitivity subsets, metrics and raw schemas, repetitions, timing orders, bootstrap protocol, simulator semantics, and Algorithms A/B/C are unchanged. Experiment 1 remains **NOT EXECUTED**, and no entry was added to `docs/EXPERIMENT_LOG.md`.
- **Verification:** The complete existing **206-test** unittest suite passed with exit code 0 in 617.255 seconds on 2026-09-17. This ran existing regressions and structural dataset-regeneration checks only; it did not execute an Experiment 1 planner workload.

## 2026-09-17 — Experiment 1 Stage 10 Measurement Infrastructure

- **Infrastructure implemented:** Added guarded `experiment-1-runner-v1` orchestration for the frozen structural, anchor, timing, decomposition, memory, and sensitivity passes. Formal execution still requires the explicit gate and invocation ID; Stage 10 did not invoke it.
- **Verified measurement mechanism:** Benchmark-only forwarding contexts call the accepted canonical visibility, supercover, Dijkstra, synchronization, and cache-installation implementations. Synthetic proofs confirm identical visible sets and planner decisions, no belief mutation, exact A/B/C call-count agreement, exact ray/full-line accounting, short-circuit interior-probe accounting, and restoration after success, exceptions, and nested use.
- **Evidence architecture:** Exact phase-aware raw schemas, canonical finite JSONL serialization, a non-overwriting invocation writer, derived summary, and immutable six-file failures are implemented. Primary correctness failure maps to `CORRECTNESS_REOPENED`; incomplete infrastructure before valid primary evidence leaves classification unavailable rather than fabricating a scientific result.
- **Analysis frozen in code:** Work/slack/storage/coverage aggregates, linear `h=(n-1)p` timing quantiles, nearest-rank slack p95, map-unit paired log-ratio bootstrap with 10,000 resamples and seed `20260916`, and every strict/equality branch of Pre-execution Amendment 1 are covered with synthetic data. Memory and sensitivity cannot override the primary decision.
- **Research conclusion unchanged:** This stage validates measurement infrastructure only. No frozen 27-map Experiment 1 planner workload ran, no Experiment 1 formal output exists, no efficiency value was observed, and the empirical-value gate remains **OPEN**. No entry was added to `docs/EXPERIMENT_LOG.md`.
