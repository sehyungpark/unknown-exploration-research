"""Structural-only frozen random-map construction for Experiment 2.
...
def dataset_id(size: int, density_label: str, accepted_index: int) -> str:
    ...
    return f"cstar-{size}x{size}-{density_label}-{accepted_index:02d}"
...
def materialize_dataset(master_seed: int = MASTER_SEED) -> Experiment2Dataset:
    ...
    return Experiment2Dataset(tuple(accepted), tuple(rejected))
...
def dataset_config(
    dataset: Experiment2Dataset, master_seed: int = MASTER_SEED
) -> dict[str, Any]:
...
def validate_config_schema(config: Mapping[str, Any]) -> None:
    ...
    if set(config) != required:
        raise ValueError("Experiment 2 dataset config top-level schema mismatch")
