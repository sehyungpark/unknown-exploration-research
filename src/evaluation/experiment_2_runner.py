"""Guarded Experiment 2 measurement runner; Stage 10 must not execute it."""
...
from .experiment_2_analysis import (
    BOOTSTRAP_RESAMPLES,
    BOOTSTRAP_SEED,
    canonical_path_hash,
...
DEFAULT_CONFIG_PATH = REPOSITORY_ROOT / "configs" / "experiment_2_efficiency_maps.json"
...
) -> tuple[Experiment2Case, ...]:
    config = load_config(config_path)
    if config["dataset_version"] != DATASET_VERSION:
        raise ValueError("formal runner requires frozen Experiment 2 dataset")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if not args.execute_preregistered:
        parser.error(
            "formal Experiment 2 is guarded; Stage 11 must explicitly pass "
...
__all__ = [
    ...
    "Experiment2Case", "Experiment2RunFailure", "MethodEpisodeResult",
